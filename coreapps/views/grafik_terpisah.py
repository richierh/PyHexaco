...
#         info._font = font
        ...
        self.Refresh()
        self.Layout()
        self.ultimateList.Bind(wx.EVT_LIST_COL_END_DRAG ,self.runnow   )
...
#                  ("jhgjgjgjgjhjgj","ddd",'sdfsfdsdfsf')]
        ...
        for item in items :
            print (item)
            item1 = wordwrap(item[0],self.ultimateList.GetColumnWidth(0),wx.ClientDC(self))
            item2 = wordwrap(item[1],self.ultimateList.GetColumnWidth(1),wx.ClientDC(self))
            item3 = wordwrap(item[2],self.ultimateList.GetColumnWidth(2),wx.ClientDC(self))
